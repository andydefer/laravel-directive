...
final class CleanLogsDirectiveTest extends IntegrationTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->logger = $this->app->make(ExecutionStatsLogger::class);
        $this->logger->setBasePath($this->tempDir);
    ...
    public function test_dry_run_with_old_files(): void
    {
        ...
        $response = $this->service->run('clean:directive-logs 30 --dry-run --verbose');
    ...
    public function test_default_days_30_with_explicit_value(): void
    {
        // Fichier de 31 jours (devrait être supprimé avec days=30)
        $this->createLogFile(31);
        $oldFile = $this->getExpectedFilePath(31);
        $this->assertFileExists($oldFile);

        $response = $this->service->run('clean:directive-logs 30 --dry-run');
    ...
    public function test_null_days_means_delete_all(): void
    {
        // Créer des fichiers
        $this->createLogFile(60);
        $oldFile = $this->getExpectedFilePath(60);
        ...
        $this->createLogFile(0);
        $recentFile = $this->getExpectedFilePath(0);
    ...
    public function test_delete_all_with_force_skips_confirmation(): void
    {
        ...
        $response = $this->service->run('clean:directive-logs --force');
